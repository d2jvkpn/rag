...
- 扫描版 PDF：直接失败，不做 OCR
- `docx`：解析正文、标题、表格；表格统一转成 Markdown 表格文本，忽略图片内容识别、批注、页眉页脚
...
- 当前实现里 `docx/pptx` 直接转成 Markdown 表格文本；`pdf` 使用 `pdfplumber` 提取普通文本并尝试抽取页内表格，正文会尽量排除表格区域以减少重复，但复杂跨页表或扫描版仍可能退化或失败
