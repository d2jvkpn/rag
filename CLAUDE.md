...
- DOCX/PPTX parsing: `parser.go` groups runs within the same paragraph without separators, and converts native OOXML tables (`w:tbl` / `a:tbl`) into Markdown table text while preserving block order.
- PDF parsing: `backend/scripts/parse_pdf.py` uses `pdfplumber` for page text extraction, removes detected table regions from the linear page text when possible, and converts page-local tables into Markdown table text; scanned PDFs and complex table layouts may still fail or degrade.
