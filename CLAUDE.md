...
- **Parser output** (`internal/llm/`): `Parse()` returns `ParseResult{Text, PageCount, Blocks}`. `Blocks []ParseBlock` carries structured units (`Text`, `SectionTitle`, `PageStart`) for Markdown and PPTX/DOCX/PDF; `nil` falls back to the flat `Text` field. `processDocument` applies `CleanText` per block, then passes blocks to `BuildChunks`.
- DOCX/PPTX parsing: `parser.go` groups runs within the same paragraph without separators, and converts native OOXML tables (`w:tbl` / `a:tbl`) into Markdown table text while preserving block order. Tables and plain paragraphs are the only elements converted; bold, italic, lists and other formatting are extracted as plain text.
